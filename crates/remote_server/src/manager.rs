        ...
        T: RemoteTransport + 'static,
    {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            ctx.background_executor()
                .spawn(async move {
                    // Run platform detection and binary check sequentially
                    // so that each step reuses the same SSH ControlMaster
                    // connection instead of opening parallel channels.
                ...
                .spawn(async move {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            // Wait for the subprocess to actually exit (with a
                            // short timeout) before emitting the failure event,
                            // so teardown happens after the child is gone. The
                            // captured status itself is not surfaced in
                            // warp-lite's connection-failed event.
