...
use crate::workflows::handlebars_parser::render_template;
...
pub fn render_tab_config(
    ...
    let rendered_title = config
        .title
        .as_deref()
        .map(|t| render_template(t, &unquoted_context));
...
fn resolve_pane_node(
    ...
    if let Some(split_direction) = &node.split {
        ...
        let cwd = node
            ...
            .as_deref()
            .map(|c| {
                let rendered = render_template(c, unquoted);
            ...
            .map(|cmds| {
                cmds.iter()
                    .map(|cmd| CommandTemplate {
                        exec: render_template(cmd, quoted),
                    })
                    .collect()
            })
            .unwrap_or_default();
